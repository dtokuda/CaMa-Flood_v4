...
function bulk_cor(Wind, Triv, Tair) result(rc)
    double precision rc
    double precision, intent(in) :: Wind, Triv, Tair
    double precision, parameter  :: CH_default = 1.2d-3
    double precision stability, stability0

    if (Wind == 0.d0) then
        rc = 0.d0
    else
        stability0 = (Triv - Tair) / (Wind * Wind)
        stability  = stability0 * (abs(stability0) / (abs(stability0) + 0.01d0))
        if (Triv < Tair) then
            if (stability < -3.3d0) then
                rc = 0.d0
            elseif (stability < 0.d0) then
                rc = 0.1d0 + 0.03d0 * stability + 0.9d0 * exp(4.8d0 * stability)
           else
                rc = 1.d0
            endif
        else
            rc = 1.d0 + 0.63d0 * sqrt(stability)
        endif
    endif
...
function sensible_coef(CHD, Wind) result(coef)
    ...
    double precision, intent(in) :: CHD, Wind
...
function latent_coef(Twat, CHD, Wind) result(coef)
    double precision coef
    double precision, intent(in) :: Twat, CHD, Wind
    double precision L
    L    = water_latent(Twat)
...
&                    Psrf, Qair, Tair, Wind, Triv, sns_only)
